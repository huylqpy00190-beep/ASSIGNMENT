...
public class ApproveReporterServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            List pending = dao.getPendingReporterRequests();
            request.setAttribute("pendingList", pending);
            request.getRequestDispatcher("/WEB-INF/views/admin/approve_reporters.jsp").forward(request, response);
        } catch (Exception e) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(email==null){ response.sendRedirect(request.getContextPath()+"/quan-ly-nguoi-dung"); return; }
        try {
            UserDAO dao = new UserDAO();
